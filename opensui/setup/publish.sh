...
publish_res=$(sui client publish --gas-budget 200000000 --skip-dependency-verification --json ../../opensui)
...
ADMIN_CAP=$(echo "$newObjs" | jq -r 'select (.objectType | contains("::pbt::AdminCap")).objectId')
PUBLISHER_PBT=$(echo "$newObjs" | jq -r 'select(.objectType | contains("::Publisher")).objectId' | tail -n 1)
UPGRADE_CAP=$(echo "$newObjs" | jq -r 'select (.objectType | contains("::package::UpgradeCap")).objectId')
PBT_ARCHIVE=$(echo "$newObjs" | jq -r 'select (.objectType | contains("::pbt::PhysicalArtifactArchive")).objectId')
...
ADMIN_CAP=$ADMIN_CAP
PUBLISHER_PBT=${PUBLISHER_PBT}
PBT_ARCHIVE=$PBT_ARCHIVE
